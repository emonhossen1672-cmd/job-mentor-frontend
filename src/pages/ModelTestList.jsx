...
import { fetchModelTests } from '../services/api'
...
export default function ModelTestList() {
  const [searchParams] = useSearchParams()
  const navigate = useNavigate()
  const examType = searchParams.get('type') || 'preli'

  const [view, setView] = useState('categories') // categories | tests
  ...
  async function openCategory(cat) {
    ...
    setLoading(true)
    try {
      const data = await fetchModelTests(cat.key, examType)
      setTests(data)
      setView('tests')
    } catch (err) {
      console.error(err)
  ...
  function startTest(test) {
    navigate(`/model-tests/${test.id}/exam`, {
      ...
      {view === 'categories' && (
        <div>
          <h1 className="text-xl font-bold mb-4">
            {examType === 'written' ? 'রিটেন মডেল টেস্ট' : 'প্রিলি মডেল টেস্ট'}
          </h1>
